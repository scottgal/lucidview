...
| `ux-scripts/run-system-chrome.sh [out]` | `verify-system-chrome.yaml` | The window chrome branch Windows and Linux take, **measured in pixels**. Runs the same script twice, once normally and once with `MYLO_FORCE_SYSTEM_CHROME=1`, and compares where the toolbar's content starts: 80px in on macOS, where the traffic lights are, and 12px in with the system title bar. It is not evidence about how Windows looks, and cannot be; it is evidence that `ConfigurePlatformChrome` runs, finds the toolbar and moves it, which is the part that would otherwise fail silently | no |
