...
public static class ReaderShortcuts
{
        ...
        KeyModifiers commandModifier)
    {
        ...
        // F1 is the one gesture that carries no modifier and is still allowed
        // while a text box has focus. It produces no character, so unlike the
        // bare letters below it cannot be generated by typing a word, and
        // help that stops working the moment the caret is in the search box
        // would be help nobody could reach when they needed it.
        if (key == Key.F1 && modifiers == KeyModifiers.None)
            return ReaderShortcut.OpenUserManual;
